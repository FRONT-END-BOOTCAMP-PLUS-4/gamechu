...
> ⚠️ **Edge case**: If user code wraps a `notFound()` call inside a `try-catch`, the special error will be swallowed _before_ reaching the error boundary — this is a code-level issue unrelated to our boundary. All current routes use standardised try-catch patterns (from `refactor/#277`) that do not catch `notFound()`.
